...
class DiffusionUnetHybridPointcloudPolicy(BasePointcloudPolicy):
    ...
    def compute_loss(self, batch):
        ...
        nobs = self.normalizer.normalize(batch['obs']) # 128 * 4 * 512* 6
        nactions = self.normalizer['action'].normalize(batch['action'])

        if not self.use_pc_color:
            nobs['point_cloud'] = nobs['point_cloud'][..., :3] # 128*4*512*3 
        
        
        if self.se3_aug is not None: # False
        ...
        if self.obs_as_global_cond: # True
            ...
            this_n_point_cloud = this_n_point_cloud[..., :3] # 128 * 2 * 512 * 3
        else:
            # reshape B, T, ... to B*T
        ...
        condition_mask = self.mask_generator(trajectory.shape) # all False ?? 128 * 4 * 4

...
def point_cloud_sampling(point_cloud:np.ndarray, num_points:int, method:str='fps'):
    ...
    if method == 'uniform':
        ...
        for i in range(point_cloud.shape[0]):
            pc = point_cloud[i][...,:3]
            pc_np = pc.cpu().numpy()
            sampled_indices_i = fpsample.bucket_fps_kdtree_sampling(pc_np, num_points)
            sampled_indices.append(sampled_indices_i)
        sampled_indices = np.stack(sampled_indices, axis=0).astype(np.int64)
        sampled_indices = torch.tensor(sampled_indices).cuda()
        # sampled_indices = fpsample.bucket_fps_kdtree_sampling(point_cloud[...,:3], num_points)

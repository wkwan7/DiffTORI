...
class TrainDiffusionUnetHybridPointcloudWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        if cfg.training.use_ema:
            ...
            # if self.model has expert_policy, copy it to ema_model
            if hasattr(self.model, 'expert_policy'):
                self.ema_model.expert_policy = self.model.expert_policy
    ...
    def run(self):
        ...
        if cfg.training.debug:
            ...
            cfg.task.env_runner.eval_episodes = 10
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if cfg.policy._target_ != "diffusion_policy_3d.policy.difftori.DiffTORI" and cfg.policy._target_ != 'diffusion_policy_3d.policy.residual_policy.ResidualPolicy':
                            # compute loss
                            ...
                            ret_dict = self.model.train_on_batch(batch)
                            train_losses.append(ret_dict['loss'])
                            step_log = {
                                'global_step': self.global_step,
                                'epoch': self.epoch,
                                'lr': lr_scheduler.get_last_lr()[0],
                            }
                            step_log.update(ret_dict)
                            if cfg.training.use_ema:
                                ema.step(self.model)
                ...
                if cfg.training.use_ema:
                    ...
                    policy.expert_policy = self.model.expert_policy
                policy.eval()
                # run rollout
                ...
                if (self.epoch % cfg.training.checkpoint_every) == 0 and cfg.checkpoint.save_ckpt:
                    ...
                    if topk_ckpt_path is not None:
                        # topk_ckpt_path = str(topk_ckpt_path).replace('=', '-')
    ...
    def eval(self):
        ...
        # files = os.listdir(pathlib.Path(self.output_dir).joinpath('checkpoints'))
        # for file in files:
        #     if 'latest' in file:
        #         continue
        #     lastest_ckpt_path = pathlib.Path(self.output_dir).joinpath('checkpoints', file)
